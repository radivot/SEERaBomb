...
levels(canc$cancer) #see all cancer types defined by mapCancs() in mkSEER()
...
dd$grp=factor(dd$grp,levels=ord) #order legend
dd$cancer=factor(dd$cancer,levels=c("AML","ALL","CML"))#keep plot order
graphics.off(); quartz(width=3.5,height=2.5) 
...
ggsave("~/Results/tutorial/seerLeu.pdf") # Fig 6
