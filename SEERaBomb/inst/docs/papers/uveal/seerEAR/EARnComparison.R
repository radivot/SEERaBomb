...
dkm=tibble(t=fit$time,y=1-fit$surv,Method="Kaplan-Meier")
(D2=d%>%mutate(status=ifelse((status==1)&(!CODS%in%c("melanoma","eye")),2,status)))
fitc=with(D2,cuminc(surv,status))
dci=tibble(t=fitc[[1]]$time,y=fitc[[1]]$est,Method="Competing Risks")
...
D1%>%ggplot(aes(x=t,y=EAR))+gp+gx +
  ...
  tc(14)+ltb+ltp
ggsave("outs/poisRegEARfig2E.pdf",width=4,height=3)

load("data/pRS.RData")
drs=pRS%>%mutate(t=x,y=1-svr,Method="Relative Survival")
# D=bind_rows(list(dkm,dci,drs,tb%>%select(t,y,Method),pS%>%select(t,y,Method)))
D=bind_rows(list(dkm,dci,drs,pS%>%select(t,y,Method)))
D%>%ggplot(aes(t,y,col=Method))+geom_step()+gx+
  ylab(" Probability of Uveal Melanoma Death")+tc(13)+ltb+
  ...
  ggsave("outs/comparisonFig2F.pdf",width=4.5,height=3.5)
