seerSet2<-function(canc,popsa,Sex, Race="pooled",ageStart=15,ageEnd=85) {
  # gimic to get rid of unwanted notes in R CMD check
   agedx=age=age86=yrdx=sex=race=surv=yrbrth=py=year=NULL 
...
#when the next 2 mutations were in one call, yrdx would occasionally overwrite surv ... very weird.
    canc=canc%>%mutate(yrdx=yrdx+0.5) 
