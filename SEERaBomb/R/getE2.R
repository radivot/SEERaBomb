...
#    EI=lapply(EI,function(x) x[,-1])
  LPYM=lapply(LPYM,function(x) x[-c(1:15,86:126),])
#   LPYM=lapply(LPYM,function(x) x[-c(1:15,86:126),-dim(x)[2]])
  ...
  for (i in picks) {   #i loop on first cancers
    ...
    { 
      if (i=="CMML" & !j%in%c("CMML","MDS"))  E[i,j]=sum(PY*EI[[j]][,as.character(1986:2011)])
      if (i=="CMML" & j=="CMML")  E[i,j]=sum(PY*EI[[j]])
      if (i=="CMML" & j=="MDS" )  E[i,j]=sum(PY[,as.character(2001:2011)]*EI[[j]])
      if (i=="MDS" & !j%in%c("CMML","MDS"))  E[i,j]=sum(PY*EI[[j]][,as.character(2001:2011)])
      if (i=="MDS" & j=="CMML")  E[i,j]=sum(PY*EI[[j]][,as.character(2001:2011)])
      if (i=="MDS" & j=="MDS" )  E[i,j]=sum(PY*EI[[j]])
      if (!i%in%c("CMML","MDS") & j=="MDS" )  E[i,j]=sum(PY[,as.character(2001:2011)]*EI[[j]])
      if (!i%in%c("CMML","MDS") & j=="CMML" )  E[i,j]=sum(PY[,as.character(1986:2011)]*EI[[j]])
